window._CCSettings = {
    platform: "win32",
    groupList: [
        "default"
    ],
    collisionMatrix: [
        [
            true
        ]
    ],
    rawAssets: {
        assets: {
            "1eAE2DyEVK34ip5as9Gq1m": [
                "resources/data/db/luke.db",
                "cc.RawAsset"
            ],
            "16V495N4dAN7z4tDjEy9pA": [
                "resources/prefab/HistoryItem.prefab",
                "cc.Prefab"
            ],
            "0727zz8PRC9boS+M8r0h49": [
                "resources/prefab/SearchItem.prefab",
                "cc.Prefab"
            ],
            "9fBGePptpBEq3H62iX3A2i": [
                "resources/sound/deal.mp3",
                "cc.AudioClip"
            ],
            "3dx6Xt7ilInYSbg9ODcsFG": [
                "resources/texture/panelbg",
                "cc.SpriteFrame",
                1
            ],
            "07tlzuD5xO3LMoX9HqMSjC": [
                "resources/texture/panelbg.png",
                "cc.Texture2D"
            ]
        },
        internal: {
            "71VhFCTINJM6/Ky3oX9nBT": [
                "image/default_btn_disabled.png",
                "cc.Texture2D"
            ],
            "b4P/PCArtIdIH38t6mlw8Y": [
                "image/default_btn_pressed.png",
                "cc.Texture2D"
            ],
            d8HsitJHxOYqo801xBk8ev: [
                "image/default_panel.png",
                "cc.Texture2D"
            ],
            cf73jxyN9Jt47QTJU6ziYh: [
                "image/default_progressbar.png",
                "cc.Texture2D"
            ],
            d608qFRoFHwbXd0Dap056i: [
                "image/default_scrollbar_vertical.png",
                "cc.Texture2D"
            ],
            "61cyPdEfRN047sDK9rO0W5": [
                "image/default_scrollbar_vertical_bg.png",
                "cc.Texture2D"
            ],
            "02delMVqdBD70a/HSD99FK": [
                "image/default_sprite_splash.png",
                "cc.Texture2D"
            ]
        }
    },
    launchScene: "db://assets/scene/LukeScene.fire",
    scenes: [
        {
            url: "db://assets/scene/LukeScene.fire",
            uuid: "a0bu+f4H1PBr3ND1rLRUmt"
        }
    ],
    packedAssets: {
        "040e0ae1e": [
            "29FYIk+N1GYaeWH/q1NxQO",
            "3dx6Xt7ilInYSbg9ODcsFG",
            "5cO7kybDxGj4ipyMYdRYZB",
            "5f5dyqtRNNxaFmVzYns6FZ",
            "675ovJ2tVK2aLYfgPUWOMv",
            "9bvaMerUlDyary99mJa6xp",
            "a0bu+f4H1PBr3ND1rLRUmt",
            "a2MjXRFdtLlYQ5ouAFv/+R",
            "e97GVMl6JHh5Ml5qEDdSGa"
        ],
        "0421e4c90": [
            "0727zz8PRC9boS+M8r0h49",
            "a2MjXRFdtLlYQ5ouAFv/+R"
        ],
        "065e89b02": [
            "16V495N4dAN7z4tDjEy9pA",
            "a2MjXRFdtLlYQ5ouAFv/+R"
        ]
    },
    orientation: "",
    debug: true
};
